...
def strat7_birdness_scores(frame_bgr, boxes_xywh, crop_scale):
    ...
    if len(tensors) == 0: return scores
    ...
    for i, img_path in enumerate(images):
        ...
        if prev_gray is not None:
            ...
            if warped_prev is not None:
                ...
                t_low = max(8.0, min(60.0, float(np.percentile(diff_hp, 98.5))))
                t_high = max(15.0, min(85.0, float(np.percentile(diff_hp, 99.6))))
                ...
                for cnt in contours:
                    ...
                    if 50 < area < 5000:
                        ...
                        if 0.2 < (float(w)/h if h > 0 else 0) < 4.0:
                ...
                if len(raw_detections) > 0:
                    ...
                    if scored:
                        ...
                        order = np.argsort([scores[i] for i in keep])[::-1][:5]
                        raw_detections = [boxes[keep[i]] + [scores[keep[i]]] for i in order]

                if config.get("use_verifier", True) and len(raw_detections) > 0:
                    bird_scores = strat7_birdness_scores(frame, raw_detections, config.get("crop_scale", 4.0))
                    final_raw = []
                    for i_score, score in enumerate(bird_scores):
                        if score >= config.get("bird_threshold", 0.01):
                            final_raw.append(raw_detections[i_score][:4] + [score])
        ...
        image_results.append(csv_utils.create_image_result(
            video_name=video_name, frame_name=frame_name, image_path=img_path,
            predictions=final_preds, ground_truths=gts, tp=img_tp, fp=img_fp, fn=img_fn,
            processing_time_sec=img_processing_time, iou=img_avg_iou, memory_usage_mb=img_mem,
        ))
...
def _run_shared_logic(config: Dict[str, Any]):
    ...
    logger.info(f"📂 Found {len(video_folders)} videos. Starting parallel processing with {Config.MAX_WORKERS} workers...")
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=Config.MAX_WORKERS) as executor:
        future_to_video = {executor.submit(process_video_worker, args): args[0] for args in worker_args}
        ...
        for future in concurrent.futures.as_completed(future_to_video):
            ...
            try:
                ...
                vis_utils.log_video_metrics(logger, result["video_name"], {
                    "n_frames": result["n_frames"], "fps": result["fps"],
                    "precision": result["precision"], "recall": result["recall"],
                    "f1_score": result["f1_score"], "tp": result["tp"],
                    "fp": result["fp"], "fn": result["fn"],
                    "mAP": result["mAP"], "dotd": result["dotd"],
    ...
    summary_metrics = {
        ...
        "mAP": overall_map, "dotd": overall_dotd,
    ...
    return {
        "pipeline": pipeline_name, "total_frames": total_frames,
        "avg_fps": avg_fps, "precision": overall_prec,
        "recall": overall_rec, "f1_score": overall_f1,
