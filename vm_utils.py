...
def shutdown_vm(delay_seconds: int = 60, force: bool = False) -> None:
    ...
    if not force:
        ...
        logger.warning(
            f"🔴 KILLSWITCH ACTIVATED - VM will shutdown in {delay_seconds} seconds"
        )
        logger.warning(f"   VM Name: {vm_name}")
        logger.warning(f"   Zone: {zone}")
    else:
        logger.warning(
            f"🔴 KILLSWITCH ACTIVATED (FORCED) - System will shutdown in {delay_seconds} seconds"
        )
...
def test_shutdown_permissions() -> bool:
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            logger.warning(
                "   Run: sudo visudo and add: <username> ALL=(ALL) NOPASSWD: /sbin/shutdown"
            )
            return False
